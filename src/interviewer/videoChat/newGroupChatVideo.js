...
import socket from '../../common/videoChat/socket';
...
function getHTMLMediaElement(mediaElement, config) {
  ...
  mediaBox.querySelector('video').style.maxHeight = 20 + 'px';
...
this.connection.onstream = function(event) {
 ...
 //     recordingStatus.innerHTML = 'Recording ' + this.connection.recorder.streams.length + ' streams';
  ...
  openRoom = () => {
    ...
    console.log('(self.connection.sessionid ',self.connection.sessionid);
    this.connection.open("abcdef", function(isRoomOpened, roomid, error) {
      
      console.log('isRoomOpened ',isRoomOpened);

      if(isRoomOpened === true) {
        self.showRoomURL("abcdef");
      }
      else {
  ...
  showRoomURL = (roomid) => {
    ...
    this.setState({urls: html});
  }


  joinRoom = () => {
    this.connection.join(document.getElementById('room-id').value, function(isJoinedRoom, roomid, error) {
      if (error) {
          //  disableInputButtons(true);
            if(error === 'Room not available') {
 ...
 render(){
  return(
       <div>
           <div id="room-urls" value={this.state.urls}></div>
            <input type="text" id="room-id" value="abcdef" autocorrect="off" autocapitalize="off"/>
         <button onClick={this.openRoom.bind(this)}>Open Room</button>
