...
class App extends Component {
  ...
  startCallInterviewer() {
    ...
    this.multiStreamRecorder.start(100000);
  }
    ...
    if (this.props.user.roleId == 2) {
      this.pc = new PeerConnection(this.props.location.state.videoKeyClient)
        .on('localStream', src => {
          ...
          this.multiStreamRecorder = new MediaStreamRecorder.MultiStreamRecorder(
            [src]
          );
  ...
  endCall(isStarter) {
    if (this.props.user.roleId == 2) this.multiStreamRecorder.stop();
